...
class BarcodeScannerView : SurfaceView {
    ...
    fun bindBarcodeScan(dependency: ActivityAccess): Observable<String> {
        val out = PublishSubject.create<String>()
        dependency.requestPermission(android.Manifest.permission.CAMERA) {
            if (it) {
                val barcodeDetector: BarcodeDetector = BarcodeDetector.Builder(dependency.context)
                    .setBarcodeFormats(Barcode.ALL_FORMATS)
                    .build()
                val cameraSource: CameraSource = CameraSource
